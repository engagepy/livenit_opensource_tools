...
def weather(city):
    ...
    r = requests.get(
        f"https://api.openweathermap.org/data/3.0/onecall?lat=33.44&lon=-94.04&exclude=hourly,minutely,pressure,uvi,icon,wind_deg&lang=hi&units=metric&appid={os.environ['openweather_key']}"
    )
    r = r.json()
    r2 = r2.json()
    ...
    format = r['current']
    format2 = format['temp']
    format3 = format['weather']
    print(f'formatted is {format2}, {format3}')
